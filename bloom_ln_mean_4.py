...
@torch.no_grad()
def bloom_sequential(model, dataloader, dev, means=None, stds=None, update_norm=False, lr=1e-6, iters=1):
    print('Starting ...')
    ...
    for i in range(len(layers)):
        ...
        if update_norm:
            ...
            _lr = lr_schedule(i)
            opt = torch.optim.Adam(norm_params, lr=_lr)
    ...
    #             quant_outs[idx].append(_out.cpu().numpy())
    #         return tmp
    # for i in range(len(layers)):
    #     quant_outs[i] = []
    ...
    #     np.save("weights_dist/bloom_output_q_ln_mean2_all/layer_{}".format(i), np.array(quant_outs[i]))
    # del quant_outs
...
if __name__ == '__main__':
    ...
    parser.add_argument('dataset', type=str, choices=['wikitext2', 'ptb', 'c4', 'rand_gen', 'qat_gen'], help='Where to extract calibration data from.')
    ...
    parser.add_argument('--iters', type=int, default=1, help='Tuning Iterations.')
    ...
    if not args.load and not args.load_hf_model and args.wbits < 16 and not args.nearest:
        tick = time.time()
        quantizers = bloom_sequential(model, dataloader, DEV, update_norm=args.update_norm, lr=args.lr, iters=args.iters)
