...
@torch.no_grad()
def bloom_sequential(model, dataloader, dev, means=None, stds=None, update_norm=False, lr=1.e-6):
    print('Starting ...')
    ...
    for i in range(len(layers)):
        ...
        # rand_inps = ( torch.randn_like(inps) * inps.std() ) + inps.mean()
        rand_inps = torch.randn_like(inps) * 1.2

        handles = []
        for name in subset:
            handles.append(subset[name].register_forward_hook(add_batch(name)))
        for j in range(args.nsamples):
            _ = layer(inps[j].unsqueeze(0), attention_mask=attention_mask, alibi=alibi)[0]
            outs[j] = layer(rand_inps[j].unsqueeze(0), attention_mask=attention_mask, alibi=alibi)[0]
        for h in handles:
            h.remove()
        ...
        if update_norm:
            ...
            loss_func = torch.nn.KLDivLoss(reduction='mean', log_target=True)
            # loss_func = torch.nn.MSELoss(reduction='sum')
            batch_size = 1
            T = 1.0

            batch_inps = torch.cat([rand_inps[t].unsqueeze(0).float() for t in range(args.nsamples)])
                ...
                for it in range(iters):
                    for j in range(args.nsamples // batch_size):
                        ...
                        total_loss += loss_func(F.log_softmax(cur_out / T, dim=-1), 
                                                F.log_softmax(batch_outs[j*batch_size : (j+1)*batch_size].detach() / T, dim=-1)) * (T * T)
    ...
    #     np.save("bloom_output_q_ln/layer_{}".format(i), np.array(quant_outs[i]))
    # del quant_outs
