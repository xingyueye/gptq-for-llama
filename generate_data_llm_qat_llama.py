...
rand_seed = 1
file_name = "llama-7b-generate_llm-qat_seed1"
random.seed(rand_seed)

from langdetect import detect

print("Loading tokenizer")
tokenizer = AutoTokenizer.from_pretrained("/mnt/dolphinfs/ssd_pool/docker/user/hadoop-automl/common/LM_data/model/llama-7b-hf/", use_fast=False)
print("Tokenizer loaded!")
print("Loading model")
model = AutoModelForCausalLM.from_pretrained("/mnt/dolphinfs/ssd_pool/docker/user/hadoop-automl/common/LM_data/model/llama-7b-hf/")
print("Model loaded!")
...
for i in range(seq):
    ...
    while isNotValid(rand_key):
        rand_key = random.choice(key_list)
    input_ids = torch.tensor([[vocab[rand_key]]]).cuda()
