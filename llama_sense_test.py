...
def get_llama(model, device_map='auto'):
    ...
    model = LlamaForCausalLM.from_pretrained(model, torch_dtype=torch.float16, device_map=device_map)
    model.seqlen = 2048
    return model

@torch.no_grad()
def llama_sense_test(args, model, model_w2, dataloader, dev, path):
    tokenizer = LlamaTokenizer.from_pretrained(args.model, trust_remote_code=True)
    tokenizer.pad_token = tokenizer.eos_token
    dataset = LLaMaLambadaDataset(args.data_path, tokenizer, split=1000)
    ...
    for i in range(len(model.model.layers)):
        layer_orin = copy.deepcopy(model.model.layers[i].state_dict())
        model.model.layers[i].load_state_dict(model_w2.model.layers[i].state_dict())
        # 测试
        tick = time.time()
        ...
        model.model.layers[i].load_state_dict(layer_orin)  # 测试结束恢复量化前的权重
        del layer_orin
...
if __name__ == '__main__':
    ...
    device_map = 'auto'
    model = get_llama(args.model, device_map=device_map)
    model.eval()

    model_w2 = LlamaForCausalLM.from_pretrained(args.load_hf_model, torch_dtype=torch.float16, trust_remote_code=True, device_map=device_map)
    model_w2.seqlen = 2048
    model_w2.eval()

    dataloader, testloader = get_loaders(args.dataset, nsamples=args.nsamples, seed=args.seed, model=args.model, seqlen=model.seqlen)
    sense_scores = llama_sense_test(args, model, model_w2, dataloader, DEV, args.sense_test)
